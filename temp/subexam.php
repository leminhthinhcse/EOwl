...
<div class="clearfix colelem" id="pu14651-4">
	<!-- group -->
	<div class="rounded-corners clearfix grpelem" id="u14651-4" style="background-color: <?php echo $mau?>; border-color: <?php echo $mau?>">
		<!-- content -->
		<p>BXH THÀNH VIÊN</p>
	</div>
...
<div class="clearfix colelem" id="ppu14641">
	...
	<div class="clearfix grpelem" id="pu14641">
		...
		<div class="clearfix colelem" id="u14646">
			<!-- group -->
			<div class="pointer_cursor clearfix grpelem" id="u14650" style="border-color: <?php echo $mau?>">
				<!-- group -->
	...
	$sql = "SELECT * FROM `exam` WHERE `subject` = '$sub' ORDER BY `exam_id` DESC";
	$query = mysqli_query( $conn, $sql );
	$data = mysqli_fetch_array( $query, MYSQLI_ASSOC );

	//TÌM TỔNG SỐ RECORDS
	$result = mysqli_query( $conn, "select count(exam_id) as total from exam WHERE `subject` = '$sub'" );
	...
	$result = mysqli_query( $conn, "SELECT * FROM exam WHERE `subject` = '$sub' ORDER BY `exam`.`exam_id` DESC LIMIT $start, $limit " );
	?>
	...
	<div class="clearfix grpelem" id="u13517">
		<table>
            ...
            if ($current_page > 1 && $total_page > 1){
                echo '<p>.</p><a href="contest.php?subject='.$subject.'&page='.($current_page-1).'">Prev</a> | ';
            }
            ...
            for ($i = 1; $i <= $total_page; $i++){
                ...
                if ($i == $current_page){
                    ...
                    echo '<a href="contest.php?subject='.$subject.'&page='.$i.'">'.$i.'</a> | ';
                }
            }
 
            // nếu current_page < $total_page và total_page > 1 mới hiển thị nút prev
            if ($current_page < $total_page && $total_page > 1){
                echo '<a href="contest.php?subject='.$subject.'&page='.($current_page+1).'">Next</a> | ';
            }
           ?>
					</div>
